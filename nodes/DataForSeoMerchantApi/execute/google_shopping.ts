...
export async function getSellersHtml(ef: IExecuteFunctions, i: number) {
	const specifications = ef.getNodeParameter('additional_specifications', i) as IDataObject;
	const parsedSpecifications = parseSpecifications(specifications);
	...
	return dataForSeoPOSTGETRequest(ef, params, '/merchant/google/sellers/task_get/html/');
}

export async function getProductSpecificationAdvnced(ef: IExecuteFunctions, i: number) {
	const specifications = ef.getNodeParameter('additional_specifications', i) as IDataObject;
	const parsedSpecifications = parseSpecifications(specifications);

	const params: IHttpRequestOptions = {
		url: '/merchant/google/product_spec/task_post',
		body: [{
			product_id: ef.getNodeParameter('product_id', i) || null,
			data_docid: ef.getNodeParameter('data_docid', i) || null,
			priority: ef.getNodeParameter('priority', i) || null,
	...
	return dataForSeoPOSTGETRequest(ef, params, '/merchant/google/product_spec/task_get/advanced/');
}

export async function getProductSpecificationHtml(ef: IExecuteFunctions, i: number) {
	const specifications = ef.getNodeParameter('additional_specifications', i) as IDataObject;
	const parsedSpecifications = parseSpecifications(specifications);
	...
	return dataForSeoPOSTGETRequest(ef, params, '/merchant/google/product_spec/task_get/html/');
}
